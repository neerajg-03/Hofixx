...
def record_deposit_transaction(provider, amount, transaction_type='credit', source='recharge', 
    ...
    if transaction_type == 'credit':
        ...
        # Allow negative balances for commission deductions
        # For other debits, still check balance (but allow going negative for commission)
        if source != 'commission_deduction' and current_balance < amount:
...
def deduct_commission(provider, booking, commission_rate=10.0):
    ...
    # Reload provider to get latest balance
    provider.reload()
    current_balance = float(provider.deposit_balance or 0.0)
    
    # Allow negative balances - commission will be deducted even if balance is insufficient
    # Provider will need to recharge to continue receiving service requests
    
    # Deduct commission (will allow negative balance)
...
def check_minimum_balance(provider, minimum_balance=50.0):
    ...
    Returns False for negative balances or balances below minimum."""
    if not provider:
    ...
    # Negative balance or balance below minimum - not eligible
...
def get_deposit_summary(provider, limit=20):
    ...
    current_balance = round(float(provider.deposit_balance or 0.0), 2)
    summary = {
        'deposit_balance': current_balance,
        'minimum_required': 50.0,
        'is_eligible': current_balance >= 50.0,
        'transactions': [{
            'id': str(tx.id),
            'amount': tx.amount,
