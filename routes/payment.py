...
from models import Booking, Payment, User, Order, ProviderPayout, Provider
...
def create_razorpay_order():
    ...
    try:
        ...
        # Get payment method (default to Razorpay, can be 'Hofix' for direct payment)
        payment_method = data.get('payment_method', 'Razorpay')
        
        # Calculate discount for Hofix payments (3% discount)
        original_amount = amount / 100  # Convert from paise to rupees
        discount_percentage = 0.0
        discount_amount = 0.0
        final_amount = original_amount
        
        if payment_method == 'Hofix':
            discount_percentage = 3.0
            discount_amount = (original_amount * discount_percentage) / 100
            final_amount = original_amount - discount_amount
        
        # Convert final amount back to paise for Razorpay
...
def verify_razorpay_payment():
    ...
    try:
        ...
        # Get payment method and discount info from order notes
        payment_method = 'Razorpay'
        original_amount = payment_details['amount'] / 100
        discount_percentage = 0.0
        discount_amount = 0.0
        final_amount = original_amount
        ...
        try:
            ...
            payment_method = notes.get('payment_method', 'Razorpay')
            if notes.get('original_amount'):
                original_amount = float(notes.get('original_amount', original_amount))
            if notes.get('discount_percentage'):
                discount_percentage = float(notes.get('discount_percentage', 0))
            if notes.get('discount_amount'):
                discount_amount = float(notes.get('discount_amount', 0))
            final_amount = original_amount - discount_amount
        except Exception as e:
            print(f"Error fetching order details: {e}")
        
        # Calculate commission (15% of final amount paid)
        commission_percentage = 15.0
        commission_amount = (final_amount * commission_percentage) / 100
        provider_payout_amount = final_amount - commission_amount
        ...
        # Create provider payout record if booking has a provider
        if booking.provider and payment.status == 'Success':
            ...
            payout = ProviderPayout(
                ...
                commission_percentage=commission_percentage,
                commission_amount=commission_amount,
                payout_amount=provider_payout_amount,
            ...
            print(f"Created payout record: {payout.id} for provider {provider.id}, amount: ₹{provider_payout_amount}")
