...
def signup():
    ...
    if role == 'provider':
        provider = Provider(user=user, skills=['Electrician', 'Plumber'], availability=True, verification_status='not_started')
        provider.save()
        ...
        try:
            ...
            socketio.emit('new_provider_registered', {
                'provider_id': str(provider.id),
                'user_id': str(user.id),
                'name': user.name,
                'email': user.email,
                'phone': user.phone,
                'message': f'New provider {user.name} has registered and needs verification'
            }, to='admin_room')
        except Exception as e:
            print(f"Error sending admin notification for new provider: {e}")
