...
from models import User, Provider, Booking, ProviderDepositTransaction
from bson import ObjectId
from datetime import datetime, timedelta
from services.provider_deposit_service import (
    resolve_provider, record_deposit_transaction, deduct_commission,
    check_minimum_balance, get_deposit_summary, ProviderDepositError
)
import math
import random
import json
...
# Razorpay configuration for provider deposits
...
provider_deposit_razorpay_client = razorpay.Client(auth=(razorpay_key_id, razorpay_key_secret))
...
def providers_nearby():
    ...
    for u in users:
        ...
        # Check verification status - only verified providers should appear
        if provider.verification_status != 'verified':
            print(f"  Provider {u.name} is not verified (status: {provider.verification_status}), skipping")
            continue
        
        # Check minimum deposit balance (₹50 required)
        from services.provider_deposit_service import check_minimum_balance
        is_eligible, _ = check_minimum_balance(provider, minimum_balance=50.0)
        if not is_eligible:
            print(f"  Provider {u.name} has insufficient balance (₹{provider.deposit_balance or 0:.2f}), skipping")
            continue
...
# Provider Deposit/Recharge Endpoints
@provider_bp.get('/api/provider/deposit')
@jwt_required()
def get_provider_deposit():
    """Get provider deposit balance and recent transactions"""
    try:
        ident = get_jwt_identity()
        provider = resolve_provider(ident)
        if not provider:
            return jsonify({'error': 'Provider not found'}), 404
        
        summary = get_deposit_summary(provider)
        ...
        import traceback
        traceback.print_exc()
        return jsonify({'error': f'Failed to get deposit info: {str(e)}'}), 500


@provider_bp.post('/api/provider/deposit/razorpay/create-order')
@jwt_required()
def create_provider_deposit_razorpay_order():
    """Create Razorpay order for provider deposit recharge"""
    try:
        ident = get_jwt_identity()
        provider = resolve_provider(ident)
        if not provider:
        ...
        receipt_id = f"provider_deposit_{str(provider.id)[:20]}_{int(datetime.utcnow().timestamp())}"
        ...
        try:
            order_data = {
                ...
                'notes': {
                    'provider_id': str(provider.id),
                    'type': 'provider_deposit_recharge'
                }
            }
            order = provider_deposit_razorpay_client.order.create(order_data)
        except Exception as e:
            error_msg = str(e)
            return jsonify({
        ...
        if not order or 'id' not in order:
            return jsonify({'error': 'Invalid response from Razorpay'}), 500
...
@provider_bp.post('/api/provider/deposit/razorpay/verify')
@jwt_required()
def verify_provider_deposit_razorpay_payment():
    """Verify Razorpay payment and credit provider deposit"""
    try:
        ident = get_jwt_identity()
        provider = resolve_provider(ident)
        if not provider:
        ...
        try:
            ...
            provider_deposit_razorpay_client.utility.verify_payment_signature(params_dict)
        except Exception as e:
            return jsonify({'error': 'Payment verification failed', 'details': str(e)}), 400
        
        # Check if transaction already processed
        existing_tx = ProviderDepositTransaction.objects(external_reference=razorpay_payment_id).first()
        if existing_tx:
            return jsonify({
        ...
        try:
            new_balance = record_deposit_transaction(
                ...
                source='recharge',
                description=f'Deposit recharge via Razorpay (Order: {razorpay_order_id})',
        ...
        return jsonify({
            'success': True,
            'message': 'Deposit recharged successfully',
