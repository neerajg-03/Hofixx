...
def notify_nearby_providers(service_request):
    ...
    try:
        ...
        for provider in providers:
            # Check verification status - only verified providers can receive requests
            if provider.verification_status != 'verified':
                print(f"Provider {provider.user.name} is not verified (status: {provider.verification_status}), skipping")
                continue
            
            # Check minimum deposit balance (₹50 required)
            is_eligible, error_message = check_minimum_balance(provider, minimum_balance=50.0)
            if not is_eligible:
                print(f"Provider {provider.user.name} has insufficient balance (₹{provider.deposit_balance or 0:.2f}), skipping")
                continue
        ...
        # If no nearby providers found, notify all eligible providers (with verification and balance checks)
        if not nearby_providers:
            print("No nearby providers found, checking all available providers")
            for provider in providers:
                # Check verification status
                if provider.verification_status != 'verified':
                    continue
                # Check minimum deposit balance
        ...
        for provider in nearby_providers:
            try:
                ...
                # Emit to specific provider room
                socketio.emit('new_service_request', notification_data, room=provider_room)
                print(f"Emitted new_service_request to room: {provider_room} for provider user: {provider.user.name}")
                
                # Also emit to all_providers room as fallback
                socketio.emit('new_service_request', notification_data, room='all_providers')
                print(f"Also emitted to all_providers room")
...
def get_provider_service_requests():
    ...
    try:
        ...
        # Check verification status - only verified providers can see requests
        if provider.verification_status != 'verified':
            return jsonify({
                'error': 'Verification required',
                'details': 'Please complete your verification to view service requests. Go to Provider Dashboard to start verification.',
