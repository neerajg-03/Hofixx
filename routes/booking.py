...
from models import Booking, Service, Provider, Payment, User, ChatMessage
...
def create_booking():
    ...
    if provider:
        ...
        if service:
            ...
            # Send notifications to matching providers (only verified with sufficient balance)
            ...
            for provider in matching_providers:
                ...
                if provider.verification_status != 'verified':
                    print(f"Provider {provider.user.name if provider.user else 'Unknown'} is not verified, skipping")
                    continue
                
                # Check minimum deposit balance (₹50 required)
                is_eligible, _ = check_minimum_balance(provider, minimum_balance=50.0)
                if not is_eligible:
                    print(f"Provider {provider.user.name if provider.user else 'Unknown'} has insufficient balance, skipping")
                    continue
...
def send_chat_message():
    ...
    try:
        ...
        # Prepare message data for response and socket emission
        message_data = {
            'id': str(message.id),
            'booking_id': booking_id,
            'sender_id': user_id,
        ...
        # Emit to booking room (both user and provider will receive)
        socketio.emit('new_message', message_data, room=f'booking_{booking_id}')
        print(f"Emitted new_message to room: booking_{booking_id}")
        
        # Also emit to user room if message is from provider
        if sender_type == 'provider' and booking.user:
            user_room = f'user_{str(booking.user.id)}'
            socketio.emit('new_message', message_data, room=user_room)
            print(f"Emitted new_message to user room: {user_room}")
        
        # Also emit to provider room if message is from user
        if sender_type == 'user' and booking.provider:
            provider_user_id = str(booking.provider.user.id) if booking.provider.user else None
            if provider_user_id:
                provider_room = f'provider_{provider_user_id}'
                socketio.emit('new_message', message_data, room=provider_room)
                print(f"Emitted new_message to provider room: {provider_room}")
...
@booking_bp.get('/api/chat/<booking_id>/messages')
@jwt_required()
def get_chat_messages(booking_id):
    ...
    try:
        ...
        if str(booking.user.id) != user_id:
            ...
            if not user.provider_profile or not booking.provider or booking.provider.id != user.provider_profile.id:
                return jsonify({'error': 'Unauthorized'}), 403
