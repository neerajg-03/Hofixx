...
from models import Feedback, User, Booking, Provider, ShopAd, Payment, Shop, ReferralRequest, ProviderPayout
...
@jwt_required(optional=True)
def admin_payouts():
    """Admin page for managing provider payouts"""
    try:
        current_user_id, user = get_user_from_token()
        
        if not current_user_id or not user or user.role != 'admin':
            token_param = request.args.get('token', '')
            next_url = '/admin/payouts'
            if token_param:
                next_url += f'?token={token_param}'
            return redirect(url_for('auth.login') + f'?next={next_url}')
        ...
        print(f"Error loading admin payouts page: {str(e)}")
        import traceback
        traceback.print_exc()
        return redirect(url_for('auth.login'))
