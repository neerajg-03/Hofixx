...
RAZORPAY_KEY_ID = os.getenv('RAZORPAY_KEY_ID', 'rzp_live_your_live_key_id_here')
RAZORPAY_KEY_SECRET = os.getenv('RAZORPAY_KEY_SECRET', 'your_live_key_secret_here')
...
def upload_service_completion():
    ...
    try:
        ...
        # If payment method is Cash, deduct commission from provider's deposit
        if booking.payment and booking.payment.method == 'Cash':
            try:
                from services.provider_deposit_service import deduct_commission
                commission_result = deduct_commission(
                    provider=provider,
                    booking=booking,
                    commission_rate=10.0  # 10% commission
                )
                print(f"Commission deducted: ₹{commission_result['commission_amount']:.2f} from provider {provider.id}")
            except Exception as e:
                print(f"Error deducting commission: {e}")
                import traceback
                traceback.print_exc()
                # Don't fail the completion if commission deduction fails, but log it
...
# Payment route moved to payment.py to avoid conflicts


# Payment verification route moved to payment.py to avoid conflicts
