...
def allowed_file(filename):
    if not filename:
        return False
    # For files without extension (like selfie from camera), allow jpg
    if '.' not in filename:
        return True  # Will default to jpg in save_verification_file
...
def save_verification_file(file, prefix):
    ...
    try:
        if not file:
            print(f"DEBUG: No file provided for {prefix}")
            return None
            
        if not file.filename:
            print(f"DEBUG: File has no filename for {prefix}")
            return None
            
        if not allowed_file(file.filename):
            print(f"DEBUG: File type not allowed for {prefix}: {file.filename}")
            return None
        ...
        if not os.path.exists(filepath):
            print(f"DEBUG: File was not saved to {filepath}")
            return None
        
        # Return relative URL (not external) for consistency
        relative_url = f"uploads/verification/{filename}"
        full_url = url_for('static', filename=relative_url, _external=True)
        print(f"DEBUG: File saved successfully for {prefix}: {relative_url}")
        return full_url
    except Exception as e:
        print(f"DEBUG: Error saving file for {prefix}: {str(e)}")
        import traceback
        traceback.print_exc()
        return None
...
def _compute_missing_provider_fields(provider):
    ...
    if not provider.bank_account_holder_name or not provider.bank_account_number or not provider.bank_ifsc_code or not provider.bank_name:
...
def get_provider_verification_status():
    ...
    try:
        ...
        return jsonify({
            ...
            'bank_ifsc_code': provider.bank_ifsc_code,
...
def submit_provider_verification():
    ...
    try:
        ...
        if 'aadhaar_front' in request.files:
            print(f"DEBUG: Processing aadhaar_front file")
            aadhaar_file = request.files['aadhaar_front']
            print(f"DEBUG: aadhaar_front file: filename={aadhaar_file.filename}, content_type={aadhaar_file.content_type}")
            aadhaar_front_url = save_verification_file(aadhaar_file, 'aadhaar_front')
            if aadhaar_front_url:
                ...
                print(f"DEBUG: aadhaar_front saved: {aadhaar_front_url}")
            else:
                print(f"DEBUG: Failed to save aadhaar_front")
        
        if 'aadhaar_back' in request.files:
            print(f"DEBUG: Processing aadhaar_back file")
            aadhaar_file = request.files['aadhaar_back']
            print(f"DEBUG: aadhaar_back file: filename={aadhaar_file.filename}, content_type={aadhaar_file.content_type}")
            aadhaar_back_url = save_verification_file(aadhaar_file, 'aadhaar_back')
            if aadhaar_back_url:
                provider.aadhaar_back_url = aadhaar_back_url
                uploaded_field = 'aadhaar_back'
                uploaded_url = aadhaar_back_url
                print(f"DEBUG: aadhaar_back saved: {aadhaar_back_url}")
            else:
                print(f"DEBUG: Failed to save aadhaar_back")
        
        if 'pan' in request.files:
            print(f"DEBUG: Processing pan file")
            pan_file = request.files['pan']
            print(f"DEBUG: pan file: filename={pan_file.filename}, content_type={pan_file.content_type}")
            pan_url = save_verification_file(pan_file, 'pan')
            if pan_url:
                provider.pan_url = pan_url
                uploaded_field = 'pan'
                uploaded_url = pan_url
                print(f"DEBUG: pan saved: {pan_url}")
            else:
                print(f"DEBUG: Failed to save pan")
        ...
        if 'selfie' in request.files:
            print(f"DEBUG: Processing selfie file")
            selfie_file = request.files['selfie']
            print(f"DEBUG: selfie file: filename={selfie_file.filename}, content_type={selfie_file.content_type}")
            selfie_url = save_verification_file(selfie_file, 'selfie')
            if selfie_url:
                provider.selfie_url = selfie_url
                uploaded_field = 'selfie'
                uploaded_url = selfie_url
                print(f"DEBUG: selfie saved: {selfie_url}")
            else:
                print(f"DEBUG: Failed to save selfie")
        ...
        if request.form.get('bank_account_holder_name'):
            provider.bank_account_holder_name = request.form.get('bank_account_holder_name')
            uploaded_field = 'bank_details'
            uploaded_url = None
        if request.form.get('bank_account_number'):
            provider.bank_account_number = request.form.get('bank_account_number')
        if request.form.get('bank_ifsc_code'):
            provider.bank_ifsc_code = request.form.get('bank_ifsc_code').upper()
        if request.form.get('bank_name'):
            provider.bank_name = request.form.get('bank_name')
        if request.form.get('bank_branch'):
            provider.bank_branch = request.form.get('bank_branch')
        ...
        if request.form.get('action') == 'submit':
            ...
            try:
                ...
                socketio.emit('provider_verification_submitted', {
                    'provider_id': str(provider.id),
                    'user_id': str(user.id),
                    'name': user.name,
                    'email': user.email,
                    'phone': user.phone,
                    'message': f'Provider {user.name} has submitted verification documents for review'
                }, to='admin_room')
            except Exception as e:
                print(f"Error sending admin notification for verification submission: {e}")
...
def get_provider_verifications():
    ...
    try:
        ...
        # Build query - prioritize pending verifications
        if status_filter == 'pending':
            providers = Provider.objects(verification_status='pending').order_by('-verification_submitted_at', '-verification_updated_at')
        elif status_filter == 'verified':
            providers = Provider.objects(verification_status='verified').order_by('-verified_at', '-verification_submitted_at')
        elif status_filter == 'rejected':
            providers = Provider.objects(verification_status='rejected').order_by('-rejected_at', '-verification_submitted_at')
        else:
            # Show all, but prioritize pending first
            providers = Provider.objects().order_by(
                '-verification_submitted_at', '-verification_updated_at'
            )
        
        verifications = []
        
        print(f"DEBUG: Status filter: {status_filter}, Found {providers.count()} providers")
        
        for provider in providers:
            try:
                # Reload provider to ensure we have latest data
                try:
                    provider.reload()
                except Exception as reload_error:
                    print(f"DEBUG: Error reloading provider {provider.id}: {reload_error}")
                    # Try to continue with existing data
                    pass
                
                if not provider.user:
                    print(f"DEBUG: Provider {provider.id} has no user, skipping")
                    continue
                
                # Get verification status - handle None values
                verification_status = provider.verification_status
                if not verification_status:
                    verification_status = 'not_started'
                
                # Only filter out 'not_started' if we're not showing all
                if verification_status == 'not_started' and status_filter != 'all':
                    print(f"DEBUG: Provider {provider.user.name} has status 'not_started', skipping (filter: {status_filter})")
                    continue
                
                # For pending filter, only show actual pending status
                if status_filter == 'pending' and verification_status != 'pending':
                    print(f"DEBUG: Provider {provider.user.name} has status '{verification_status}', not 'pending', skipping")
                    continue
                
                print(f"DEBUG: Including provider {provider.user.name} with status '{verification_status}'")
                        
                missing_fields = _compute_missing_provider_fields(provider)
                verifications.append({
                    'provider_id': str(provider.id),
                    'user_id': str(provider.user.id),
                    'name': provider.user.name or 'Unknown',
                    'email': provider.user.email or 'N/A',
                    'phone': provider.user.phone or 'N/A',
                    'verification_status': verification_status,
                    'aadhaar_front_url': provider.aadhaar_front_url,
                    'aadhaar_back_url': provider.aadhaar_back_url,
                    'pan_url': provider.pan_url,
                ...
                print(f"DEBUG: Error processing provider {provider.id if provider else 'unknown'}: {provider_error}")
                import traceback
                traceback.print_exc()
                # Skip this provider and continue with others
                continue
        
        print(f"DEBUG: Returning {len(verifications)} verifications")
